...
{
    echo "<script>alert('File must not be empty');window.location = '../update.php'</script>";
    exit(); 
}
...
function validateExtension($uploaded_file_name) {
    ...
    if (count($file_array) > 2) { // jika terdapat lebih dari satu extension
        if ($file_array[1] !== "jpg" or $file_array[1] !== "png" or $file_array[1] !== "jpeg"){
...
function validateFileSize($uploaded_file_size) {
    if ($uploaded_file_size > 1000000) {
        echo "<script>alert('File must be 1MB or less in size.');window.location = '../update.php'</script>";
        exit();
    }
}
...
function moveAndRenameFile($tmp, $upload_location, $file_name){
    ...
    if (move_uploaded_file($tmp, $base_file)) {
        ...
        echo "<script>alert('There was an error uploading your file.');window.location = '../update.php'</script>";
        exit();
    }   
}
...
function validateHeader($file){
    if ( $f = fopen($file, 'rb') ) 
    {
        ...
        if( strncmp($header, "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a", 8) != 0 )
        {
            ...
            {
                unlink($file);
                echo "<script>alert('File format is not PNG or JPG/JPEG!');window.location = '../update.php'</script>";
                exit();
            } 
        }
    } 
...
echo "<script>alert('You have successfully updated your profile picture!');window.location = '../update.php'</script>";
?>